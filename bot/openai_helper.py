...
class OpenAIHelper:
    ...
    async def transcribe(self, filename):
        ...
        try:
            groq_client = AsyncOpenAI(api_key=os.environ.get("GROQ_API_KEY"), base_url="https://api.groq.com/openai/v1")
            with open(filename, "rb") as audio_file:
                result = await groq_client.audio.transcriptions.create(model="whisper-large-v3", file=audio_file, response_format="text")
                text = getattr(result, 'text', result)
                if isinstance(text, str):
                    # Счищаем спецсимволы, от которых Телеграм падает в обморок
